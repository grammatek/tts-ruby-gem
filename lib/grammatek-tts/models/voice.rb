  ...
  class Voice
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Grammatek::TTS::Voice` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Grammatek::TTS::Voice`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'supported_engines')
        if (value = attributes[:'supported_engines']).is_a?(Array)
          self.supported_engines = value
        end
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          supported_engines == o.supported_engines &&
          voice_id == o.voice_id
    end
    ...
    def hash
      [gender, language_code, language_name, supported_engines, voice_id].hash
    end
